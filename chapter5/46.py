...
for sentence in sentences:
    for chunk in sentence:
        ...
        particles = ' '.join([sentence[idx].find_pos('助詞')[-1].base for idx in chunk.srcs if sentence[idx].is_include_pos('助詞')])
        patterns = ' '.join([''.join([morph.surface for morph in sentence[idx].morphs]) for idx in chunk.srcs if sentence[idx].is_include_pos('助詞')])
